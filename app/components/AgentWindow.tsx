...
export function AgentWindow(props: {
  endpoint: string,
  ...
  const { messages, input, setInput, handleInputChange, handleSubmit, isLoading: chatEndpointIsLoading, setMessages } =
    useChat({
      ...
      onResponse(response) {
        ...
        console.log("sourcesHeader", sourcesHeader, "sources", sources, "messageIndexHeader", messageIndexHeader);
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    console.log("showIntermediateSteps:", showIntermediateSteps, "intermediateStepsLoading:", intermediateStepsLoading, "input:", input, "messages:", messages, "endpoint:", endpoint);
    ...
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow overflow-hidden ${(messages.length > 0 ? "border" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          ...
          />
          <button type="submit" className="shrink-0 px-8 py-4 bg-blue-600 rounded w-28 text-white">
